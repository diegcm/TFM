...
class robot_env(gym.Env):
    def __init__(self):
        ...
        self.base_ini_pos = np.array([0.0, 0.0, 1.1])
       ...
       'parentFramePos':[], 'parentFrameOrn':[], 'parentIndex':[]}
        ...
        self.sigma = 0.3 #Sigma inicial
        self.total_reward = 0
        self.total_steps = 0 #total steps performed

        #Control PID
        self.pid = PID(Kp=-50000, Ki=0, Kd=1000, setpoint=0) #Kp=-50000, Kd=1000
        self.pid.output_limits = (-50000, 50000)  # Limita la fuerza
    ...
    def step(self, action):
        ...
        # Calcular la posición deseada en z para la trayectoria sinusoidal
        desired_z = 1.1 + 0.2 * math.sin(4 * math.pi * self.steps / 250)
        #print('\ndesired_z: ', desired_z)
        
    

        #Control PID plataforma 
        # Obtener la posición actual de la plataforma en el eje Z
        platform_position = p.getLinkState(self.robot, 1)[0][2]  # Índice 0 para el joint prismatic en Z
        #print('\nplatform_position: ', platform_position)

        error = desired_z-platform_position
        #print('\n', error)

        force = self.pid(error)
        #print('\nForce: ', force)
        ...
        joint_states = p.getJointStates(self.robot, range(2, 8))
        ...
        #print('Euclid pos: ', euclid_pos)
        self.steps+=1
        self.total_steps+=1
        #Calcula sigma:
        self.calcula_sigma(sigma_min=0.05, reduction_rate=0.5, reward_threshold=40, check_interval=250)
    ...
    def reset(self):
        ...
        self.robot = p.loadURDF(fileName="C:/Users/diego/Documents/MIAR/TFM/models/x100_with_ur5/urdf/ur5e_with_platform.urdf", 
                    basePosition=self.base_pos, baseOrientation=self.base_orn)


        #Crear planos que delmitan al altura máxima y mínima de la plataforma
        prisma_superior_shape = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.5, 0.5, 0.009])
        prisma_superior_id = p.createMultiBody(0, -1, prisma_superior_shape, [0., 0., 1.3])
        p.changeVisualShape(prisma_superior_id, -1, rgbaColor=[1, 0, 0, 1])  # Rojo

        prisma_inferior_shape = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.5, 0.5, 0.009])
        prisma_inferior_id = p.createMultiBody(0, -1, prisma_inferior_shape, [0., 0., 0.9])
        p.changeVisualShape(prisma_inferior_id, -1, rgbaColor=[1, 0, 0, 1])  # Rojo


        if self.nb_episodes == 0:
            self.num_joints = 6
            #print('Num joints: ', p.getNumJoints(self.robot))
            #Diccionario articulaciones
            for joint in range (0, self.num_joints+2):
            ...
            #print(self.df_manipulator_parameters)
        


        p.setGravity(0, 0, -9.81)
        
        #Se inicializan las artculaciones con fuerza 0
        for joint in range (2, self.num_joints+2):
        ...
        self.euclid_cero_recompensa = self.euclid(self.obs[0], self.goal[0])
    ...
    def command_arm(self, action):
        for joint_index, pos in zip(range(2, 8), action):
    ...
    def reward_function(self, goal, obs, euclid_pos, quaternion, flag, ang_vel_array):
        ...
        if flag['valor'] == True:
            #print(f"\nRegión prohibida. Causa: {flag['causa']}")
            if flag['indice'] == 6:
                reward = -150
            else:
                reward = -100
