...
class DDPGAgent_her(Agent):
    """Write me
    """
    def __init__(self, nb_actions, actor, critic, critic_action_input, memory,
                 gamma=.99, batch_size=64, nb_steps_warmup_critic=1000, nb_steps_warmup_actor=1000,
    ...
    def select_action(self, state):
        batch = self.process_state_batch(state) #Cambiado por mí. Original: [state]
        #print('\nbatch: ', batch) #Cambiado por mí. Antes: nada.
    ...
    def forward(self, state):
        #print('\nstate: ', state) #Cambiado por mí. Antes: nada.
        # Select an action.
        state = self.memory.get_recent_state(state)
        #print('\nobservation: ', observation) #Cambiado por mí. Antes: nada.
    ...
    def backward(self, reward, next_state,  goal, flag, ang_vel_array, info, terminal=False): #self, state, action, reward, next_state, terminal, goal, training=True
        # Store most recent experience in memory.
        if self.step % self.memory_interval == 0:
            self.memory.append(self.recent_state, self.recent_action, reward, next_state, terminal, goal, flag, ang_vel_array, info,
        ...
        if can_train_either and self.step % self.train_interval == 0:
            ...
            for e in experiences:
                ...
                flag_batch.append(e.flag)
                ang_vel_array_batch.append(e.ang_vel_array)
                info_batch.append(e.info)
            ...
            if self.step > self.nb_steps_warmup_critic:
                #print('\nActualiza critic') #Cambiado por mi. Antes: nada.
                ...
                #print('state0_batch_with_action: ', state0_batch_with_action)
            ...
            if self.step > self.nb_steps_warmup_actor:
                #print('\nActualiza actor') #Cambiado por mi. Antes: nada.
